...
function BottomNavigation() {
  return (
    <nav className="lg:hidden fixed -bottom-1 left-1/2 transform -translate-x-1/2 z-50">
      <motion.div
        className="px-6 py-5 border bg-white/10 border-amber-100/10 border-b-0  backdrop-blur-sm rounded-t-2xl "
        ...
        <ul className="flex space-x-6">
          {navLinks.map((link, index) => (
            ...
            >
              <Link
                href={link.href}
                className="px-3 py-2 text-sm text-green-500 font-normal hover:bg-black/10 hover:border hover:border-white/20 rounded-lg transition-all"
              >
                {link.name}
              </Link>
            </motion.li>
...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      <body className={`${poppins.className} bg-primary text-white font-light`}>
