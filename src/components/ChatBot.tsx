...
const ChatBot = ({
  ...
  return (
    <>
      {/* Chat Window */}
      <div
        className={`fixed bottom-20  right-6 z-50 w-96 max-w-[calc(100vw-3rem)] transition-all duration-200 transform ${
          isOpen
          ...
          {!isMinimized && (
            <div className="flex-1 overflow-y-auto scrollbar-hide p-4 space-y-3 h-[400px] bg-gradient-to-br from-black/20 to-black/20 backdrop-blur-md">
              {messages.map(message => (
                ...
                >
                  <div
                    className={`max-w-[75%] px-4 py-3 relative shadow-md ${
                      message.sender === 'user'
